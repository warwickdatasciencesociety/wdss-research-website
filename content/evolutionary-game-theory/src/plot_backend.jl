...
function ternary_plot(;
    ...
    text_angle = 60 / (0.5 + (size[1] / (2 * size[2])))
    ...
    if grid_minor
        if grid_minor_A
            for i in 0.1:0.2:0.9
                start_point = tern2cart(1 - i, i, 0)
                end_point = tern2cart(1 - i, 0, i)
                Plots.plot!(p, [start_point[1], end_point[1]], [start_point[2], end_point[2]], colour=:black, alpha=0.2, linewidth=0.5)
            end
        end
        if grid_minor_B
            for i in 0.1:0.2:0.9
    ...
    annotate!(p, textpos[1], textpos[2], Plots.text(labels[:C], 10, :dark, rotation=text_angle))
    ...
    textpos = ((pos .+ pos2) ./ 2) .+ [dist_from_graph, dist_from_graph]
    plot!(p, [pos[1], pos2[1]], [pos[2], pos2[2]], arrow=true, colour=:black)
    annotate!(p, textpos[1], textpos[2], Plots.text(labels[:B], 10, :dark, rotation=-text_angle))
